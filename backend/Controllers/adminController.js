...
import { adminDataSchema } from "../Database/auth/UserSchema.js";
import jwt from "jsonwebtoken";
import dotenv from "dotenv";
dotenv.config();

export const adminLoginPostReq = async (req, res) => {
  try {
    let key = process.env.SECRETKEY;
    const { username, password } = req.body;
    if (username && password) {
      var check = await adminDataSchema.findOne({ username, password });
      if (check) {
